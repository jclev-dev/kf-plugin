...
source of truth.** n8n is reduced to four dumb, credentialed API tools you call.
...
```
CSV/Excel ─► you create Airtable rows ─► you call n8n tools per batch ─►
...
- **n8n tools** only make the credentialed external calls (Apify, Reoon, GoHighLevel, Instantly). They never touch Airtable and never chain to each other.
...
1. `references/setup.md` — prerequisites. If the n8n MCP or Airtable connector
   isn't available, or tool credentials aren't attached, **stop and surface that
...
3. `references/tools.md` — the four n8n tool workflow IDs, their exact inputs,
   the response envelope, and how to call them.
...
- **Treat the tool envelope as fallible.** Every tool returns
  `{ok, data, error}`. Always parse it. On `429`/`5xx`, retry with backoff and
...
5. Process in batches of ~20. Per batch, per in-scope stage: call the tool →
